/**
 * BOLÃO MIHA 2026 — Gerador do Google Form do MATA-MATA (R32 / 16 avos de final)
 * ---------------------------------------------------------------------------------------
 * COMO USAR (leva ~3 min):
 *  1) Abra https://script.google.com  ->  Novo projeto.
 *  2) Apague tudo e cole ESTE arquivo inteiro.
 *  3) Menu: Executar -> criarFormularioR32. Autorize quando pedir.
 ...
 *
 * Os confrontos já vêm com as SELEÇÕES REAIS do R32, na ordem/mando dos slots R32-01..16.
 * NÃO reordene os jogos (o "Jogo N" tem que casar com o slot R32-NN).
 *
 * REGRA DE PRAZO (combinada): o jogo de HOJE (África do Sul × Canadá) fecha ao MEIO-DIA;
 * os outros 15 jogos fecham às 23h de HOJE (28/06). Quem não mexer num jogo (deixar em
 * branco ou "Manter meu palpite original") fica com o placar da APOSTA ORIGINAL da planilha.
 * A trava real é feita pelo robô (o Google Forms não fecha pergunta a pergunta).
 */

// ====== EDITE AQUI (só se precisar) ======
var FASE = 'Mata-mata · 16 avos de final (R32)';

// Pasta do Google Drive onde o Form e a planilha de respostas serão salvos.
var PASTA_ID = '16F9MjNdz7dIZqmbZjNOSxCEkj1K9-nph';

// Confrontos REAIS do R32 — ordem = slot R32-NN, home = mandante (orientação do bracket).
var MATCHES = [
  { n: 1,  home: 'Alemanha',        away: 'Paraguai',      quando: '29/06 · 17h30' }, // R32-01
  { n: 2,  home: 'França',          away: 'Suécia',        quando: '30/06 · 18h00' }, // R32-02
  { n: 3,  home: 'África do Sul',   away: 'Canadá',        quando: '28/06 · 16h00 (HOJE)' }, // R32-03
  { n: 4,  home: 'Holanda',         away: 'Marrocos',      quando: '29/06 · 22h00' }, // R32-04
  { n: 5,  home: 'Portugal',        away: 'Croácia',       quando: '02/07 · 20h00' }, // R32-05
  { n: 6,  home: 'Espanha',         away: 'Áustria',       quando: '02/07 · 16h00' }, // R32-06
  { n: 7,  home: 'EUA',             away: 'Bósnia',        quando: '01/07 · 21h00' }, // R32-07
  { n: 8,  home: 'Bélgica',         away: 'Senegal',       quando: '01/07 · 17h00' }, // R32-08
  { n: 9,  home: 'Brasil',          away: 'Japão',         quando: '29/06 · 14h00' }, // R32-09
  { n: 10, home: 'Costa do Marfim', away: 'Noruega',       quando: '30/06 · 14h00' }, // R32-10
  { n: 11, home: 'México',          away: 'Equador',       quando: '30/06 · 22h00' }, // R32-11
  { n: 12, home: 'Inglaterra',      away: 'RD Congo',      quando: '01/07 · 13h00' }, // R32-12
  { n: 13, home: 'Argentina',       away: 'Cabo Verde',    quando: '03/07 · 19h00' }, // R32-13
  { n: 14, home: 'Austrália',       away: 'Egito',         quando: '03/07 · 15h00' }, // R32-14
  { n: 15, home: 'Suíça',           away: 'Argélia',       quando: '03/07 · 00h00' }, // R32-15
  { n: 16, home: 'Colômbia',        away: 'Gana',          quando: '03/07 · 22h30' }, // R32-16
];
...
function criarFormularioR32() {
  var form = FormApp.create('Bolão Miha 2026 — ' + FASE);
  form.setDescription(
    '🏆 Chegou o MATA-MATA! Agora cada jogo vale, e você pode mudar (ou manter) seus palpites.\n\n' +
    ...
    '⏰ PRAZO (hoje, 28/06): o jogo de HOJE — África do Sul × Canadá — fecha ao MEIO-DIA. ' +
    'Os outros 15 jogos fecham às 23h de HOJE. Pode reenviar quantas vezes quiser: vale o último ' +
    'envio dentro do prazo de cada jogo.\n\n' +
  ...
  // Os 16 jogos — nomes reais + data/hora; em branco = mantém o original
  var sl = _scorelines();
  MATCHES.forEach(function (m) {
    form.addListItem()
      .setTitle('Jogo ' + m.n + ' — ' + m.home + ' × ' + m.away)
      .setHelpText('Placar ' + m.home + ' (casa) × ' + m.away + '. Fecha ' + m.quando + '. ' +
  ...
  form.setConfirmationMessage(
    '✅ Recebido! Seus palpites do mata-mata estão salvos.\n' +
    'Quer mudar algo antes do prazo do jogo? Use o MESMO link e edite sua resposta — vale a última.\n' +
  ...
  Logger.log('Depois: PLANILHA -> Arquivo -> Compartilhar -> Publicar na web -> aba de respostas + CSV -> Publicar. Mande a URL .csv.');
}
